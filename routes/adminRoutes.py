...
async def verifyAdminToken(xAdminToken: str = Header(None)):
    ...
    # 서버에 비밀키 세팅이 누락된 경우
    if not secretKey:
        raise HTTPException(status_code=500, detail="Server Configuration Error: Secret key is missing")
...
adminRouter = APIRouter(
    route_class=CustomErrorRoute,              
    dependencies=[Depends(verifyAdminToken)]   
)

# POST /admin/licenses
adminRouter.add_api_route(
    "/licenses", 
    adminControllers.createNewLicense, 
    methods=["POST"]
)

adminRouter.add_api_route(
    "/licenses", 
    adminControllers.getAllLicenses, 
    methods=["GET"]
)
...
adminRouter.add_api_route(
    "/licenses/{licenseKey}", 
    adminControllers.deleteLicense, 
    methods=["DELETE"]
)
...
adminRouter.add_api_route(
    "/licenses/{licenseKey}/reset", 
    adminControllers.resetLicenseHw, 
    methods=["POST"]
)