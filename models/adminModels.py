...
async def createLicense(db: AsyncSession, licenseKey: str, maxDevices: int, expireDate: Optional[datetime]):
    query = text("""
...
async def updateExpireDate(db: AsyncSession, licenseKey: str, newExpireDate: datetime):
    query = text('UPDATE "subscriptions" SET "expireDate" = :newExpireDate, "updatedAt" = NOW() WHERE "licenseKey" = :licenseKey')
    result = await db.execute(query, {"licenseKey": licenseKey, "newExpireDate": newExpireDate})
    await db.commit()
    return result.rowcount

async def resetHwIds(db: AsyncSession, licenseKey: str):
    # JSONB 타입을 빈 배열로 초기화
    query = text('UPDATE "subscriptions" SET "hwIds" = \'[]\'::jsonb, "updatedAt" = NOW() WHERE "licenseKey" = :licenseKey')
    result = await db.execute(query, {"licenseKey": licenseKey})
    await db.commit()
    return result.rowcount